            ...
            case (r_state)
                ...
                START_BIT: begin
                    if (r_baud_tick) begin
                        ...
                        if (r_baud_tick_cnt == 4'd7) begin //중간 지점에서 정렬 후 data bits로
                            r_state <= DATA_BITS;
                            r_bit_cnt <= 4'd0;
                            r_baud_tick_cnt <= 4'd0;
                        end
                ...
                DATA_BITS: begin
                    if (r_baud_tick) begin
                        r_baud_tick_cnt <= r_baud_tick_cnt + 1;
                        if (r_baud_tick_cnt == 4'd15) begin //15까지 count 다 해줬으면 
                            r_data_reg[r_bit_cnt] <= rx; //샘플링
                            r_baud_tick_cnt <= 4'd0;
                            if (r_bit_cnt == 4'd7) begin //8bit 다 채우면 다음으로.
                                r_state <= STOP_BIT;
                            end else begin
